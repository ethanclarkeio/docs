...
title: "AI Gateway"
sidebarTitle: "개요"
description: "DGrid AI Gateway는 200개 이상의 주요 AI 모델을 단일 OpenAI 호환 API로 호출할 수 있는 통합 게이트웨이로, 암호화폐 결제, 스마트 라우팅, 글로벌 엣지 노드 액세스, 장애 조치, 사용량 모니터링 같은 핵심 기능을 제공합니다."
---
...
<Tip>
사용 가능한 모델 용량을 자동으로 선택하는 무료 라우트를 원하시나요? [무료 모델 라우터](/ko/ai-gateway/free-models-router)를 참고하세요.
</Tip>
...
1. <strong>선택적 헤더</strong>: `HTTP-Referer`와 `X-Title` 헤더는 선택 사항이지만, 입력하면 DGrid가 사용자의 애플리케이션을 더 잘 식별하고 최적화된 서비스를 제공하는 데 도움이 됩니다.
2. <strong>모델 명명 형식</strong>: `model` 매개변수는 `[provider]/[model-name]` 형식(예: `openai/gpt-4o`)을 사용하며, DGrid가 지원하는 모든 모델에서 일관되게 적용되어 손쉽게 전환할 수 있습니다.
3. <strong>SDK 개발 현황</strong>: DGrid 공식 SDK는 개발 중이며, 더 많은 네이티브 기능과 최적화된 성능을 제공할 예정입니다. 출시 정보는 DGrid 공식 문서 업데이트를 참고해 주세요.
